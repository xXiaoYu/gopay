...
// 微信订单退款申请
func RefundAppleOrder(mchAppid, mchid, key string, conn *HTTPSClient, refund common.Refund) (map[string]string, error) {
	var m = make(map[string]string)
	m["appid"] = mchAppid
	m["mch_id"] = mchid
	...
	m["total_fee"] = WechatMoneyFeeToString(refund.TotalFee)
	m["refund_fee"] = WechatMoneyFeeToString(refund.RefundFee)
	if refund.RefundDesc != ""{
		m["refund_desc"] = refund.RefundDesc
	}
	if refund.NotifyUrl != "" {
		m["notify_url"] = refund.NotifyUrl
	}
